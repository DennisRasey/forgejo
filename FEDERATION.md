...
*This describes Forgejo's future federation capabilities, not what it can do currently.*

Forgejo is federated using [ActivityPub](https://www.w3.org/TR/activitypub/) and the [ForgeFed extension](https://forgefed.org/) so you can interact with users and repositories from other instances as if they were on your own instance. By using the standardized ActivityPub protocol, users on any fediverse software such as [Mastodon](https://joinmastodon.org/) can follow Forgejo users, star repositories, receive activity updates, and comment on issues.

C2S ActivityPub is not supported because Forgejo already has an existing API.

## Following

You can use any fediverse software to follow a Forgejo user. Forgejo will automatically accept follow requests. The usernames of remote users are displayed as `username@instance.com`. To follow a remote user, click follow on their profile page, and a pop-up box will appear for you to type in your instance. You are redirected to your own instance, where the remote user is fetched and rendered, and you can now follow them.

When following a Forgejo user, you will receive updates when they star a repo, create, fork, or make a private repo public, or follow a user. If you are using Mastodon or Pleroma, these will show up in your feed.
...
If you change your username or the name of a repository, Forgejo handles this similarly to how Mastodon does. Forgejo will send a `Move` activity to your followers and update your actor to point to the new actor and the new actor to point to the old actor.
